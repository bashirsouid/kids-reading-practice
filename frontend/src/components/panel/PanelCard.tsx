...
interface PanelCardProps {
   ...
   /** Cache key for busting image cache (timestamp when panel was regenerated). */
   cacheKey?: number;
   showImageStatus?: boolean;
  }
 
 export function PanelCard({ panel, index, jobId, onClick, onRegenerate, isGenerating, generationProgress, cacheKey, showImageStatus = true }: PanelCardProps) {
   ...
   // Add cache-busting query param when cacheKey is provided (panel was regenerated)
   const imageUrl = !isGenerating ? (
     panel.image || (jobId && hasImage ? getPanelImageUrl(jobId, index) + (cacheKey ? `?t=${cacheKey}` : '') : null)
   ) : null;
     ...
     >
       ...
       {hasImage && onRegenerate && !isGenerating && (
         <button
           ...
           onClick={(e) => {
             e.stopPropagation();
             onRegenerate(index, panel.image_prompt);
           }}
